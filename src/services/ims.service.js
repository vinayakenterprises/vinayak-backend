...
class ImsService {
  ...
  async updateMaterial(id, updateData) {
    ...
    try {
      ...
      const newCategoryId =
        category_id !== undefined ? Number(category_id) : currentMaterial.category_id;
  ...
  async processQualityCheck(data) {
    ...
    if (totalProcessed === 0)
      throw new Error("You must provide either an approved or rejected quantity.");
    ...
    try {
      ...
      if (inventoryRows.length === 0)
        throw new Error("No pending quality stock found for this material.");

      const currentPending = inventoryRows.reduce((sum, r) => sum + Number(r.quantity), 0);
      if (currentPending < totalProcessed) {
        throw new Error(
          `Cannot process ${totalProcessed}. Only ${currentPending} available in pending.`
        );
      }
  ...
  async updateDailyStockCount(data) {
    ...
    try {
      ...
      const previousStock =
        inventoryResult.rowCount > 0 ? Number(inventoryResult.rows[0].quantity) : 0;
      ...
      const inventory = await client.query(
        `
            ...
            RETURNING *;
            `,
        [Number(material_id), currentStock, status, updated_by]
      );
      ...
      if (difference !== 0) {
        ...
        await client.query(
          `
                ...
                `,
          [Number(material_id), difference, updated_by]
        );
        ...
        if (flowType) {
          await client.query(
            `
                    ...
                    `,
            [Number(material_id), flowType, difference, updated_by]
          );
        }
      }
  ...
  async checkCategoryStockLevelsAndNotify() {
    ...
    try {
      ...
      console.log(
        `CRON: Successfully sent ${alerts.length * purchaseExecutives.length} stock status alerts.`
      );
    } catch (error) {
  ...
  async importPurchaseData(purchase) {
    const { poid, material_name, purchased_qty, purity, purchased_date, json } = purchase;
